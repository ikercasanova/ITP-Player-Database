...
const Testing = {
  ...
  renderSetup() {
    ...
    const testOptions = Object.entries(TEST_DEFS).map(([key, def]) =>
      `<option value="${key}">${def.name} (${def.unit})</option>`
    ).join('');
  ...
  renderRecentSessions() {
    // Quick summary of which tests have data
    ...
    const rows = Object.entries(counts).map(([key, count]) => {
      ...
      return def ? `<div class="settings-row">
        <span class="settings-label">${def.name}</span>
        <span class="settings-value">${count} player${count !== 1 ? 's' : ''}</span>
      </div>` : '';
    }).join('');
  ...
  startSession(container) {
    const testKey = container.querySelector('#test-select').value;
    if (!testKey) {
    ...
    Testing.activeTest = testKey;
    Testing.ageGroups = groups;
    Testing.currentIndex = 0;
    Testing.phase = 'queue';

    // Get players matching selected age groups
  ...
  renderQueue() {
    ...
    const attempts = testData?.attempts || [null, null, null];
  ...
  saveCurrentPlayer(container) {
    ...
    inputs.forEach(inp => {
      ...
      DB.updateTestResult(player.id, Testing.activeTest, idx, isNaN(num) ? null : num);
    });
  ...
  renderSummary() {
    ...
    const results = Testing.players
      .map(p => {
        ...
        return { player: p, best: t?.best ?? null };
      })
      .filter(r => r.best !== null);

    // Sort: for lowerIsBetter sort ascending, else descending
    ...
    container.innerHTML = `
      <div class="testing-page">
        <div class="testing-summary">
          <h2>Session Complete: ${def.name}</h2>
